...
export const AppHeader: React.FC<AppHeaderProps> = ({
  ...
  return (
    <div
      className="sticky top-0 z-10 grid w-full grid-cols-[auto_1fr_auto] items-center justify-end justify-items-center bg-stone-50 px-4"
      style={{ height: `var(${appHeaderHeightName})` }}
    >
      <div className="flex items-center gap-1">
        <motion.div
          className={clsx(isSidebarOpen ? "opacity-0" : "opacity-100")}
          initial={{ opacity: 0, x: -20 }}
          animate={
            isSidebarOpen ? { opacity: 0, x: -20 } : { opacity: 1, x: 0 }
          }
          exit={{ opacity: 0, x: -20 }}
        >
          <Tooltip
